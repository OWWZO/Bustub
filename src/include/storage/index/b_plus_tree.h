...
class BPlusTree {
 ...
 public:
  ...
  auto IsDistributeForInternal(BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator> *internal_write, int to_size)
      -> page_id_t;

  void CheckForLeaf(WritePageGuard &leaf_guard);

  void DeepDeleteOrUpdate(const KeyType &key, std::optional<KeyType> update_key,
                          BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator> *internal_write, bool is_update);
  ...
  auto SplitForInternal(BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator> *first_internal_write,
