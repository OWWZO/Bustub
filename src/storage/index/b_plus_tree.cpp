...
void BPLUSTREE_TYPE::UpdateFather(KeyType first_key, KeyType second_key, WritePageGuard &write_guard) {
  auto page_base = write_guard.As<BPlusTreePage>();
  bool is_leaf = page_base->IsLeafPage();

  page_id_t father_id;
  page_id_t child_page_id;

  if (is_leaf) {
    auto leaf = write_guard.AsMut<B_PLUS_TREE_LEAF_PAGE_TYPE>();
    father_id = leaf->GetFatherPageId();
    child_page_id = leaf->GetPageId();
  } else {
    auto internal = write_guard.AsMut<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
    father_id = internal->GetFatherPageId();
    child_page_id = internal->GetPageId();
  }
  ...
  if (index == -1) {
    index = father_write->ValueIndexForPage_id_t(child_page_id);
  }
  if (index == -1) {
    return;
  }

  father_write->UpdateKey(index, std::make_optional(second_key));
...
void BPLUSTREE_TYPE::RecursiveUpdateKeyForRedistribute(
    KeyType old_key, std::pair<KeyType, page_id_t> new_pair,
  ...
  if (index == -1) {
    index = father_write->ValueIndexForPage_id_t(new_pair.second);
  }
  if (index == -1) {
    return;
  }
  ...
  // 更新父页中的键值对
  father_write->UpdateKey(index, std::make_optional(new_pair.first));
  father_write->UpdateKey(index, new_pair.second);
  ...
  if (index == 0) {
    ...
    if (grandfather_id != INVALID_PAGE_ID) {
      ...
      auto grandfather_write =
          grandfather_guard.template AsMut<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
      // 递归向上更新：使用新的键值继续向上查找和更新
      auto father_new_min_key = father_write->KeyAt(0);
      RecursiveUpdateKeyForRedistribute(
          father_old_min_key, std::make_pair(father_new_min_key, father_write->GetPageId()), grandfather_write);
    }
  }
}
  ...
  if (!root_page->IsLeafPage()) {
    {
      ...
      while (!is_leaf) {
        ...
        auto temp_write_internal_page =
            guard_while.template As<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
        // 继续在此内页上查找
...
// 适用于当上层是内页 下面是叶子页的情况
FULL_INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::SplitForInternal(BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator> *first_internal_write,
...
FULL_INDEX_TEMPLATE_ARGUMENTS  // TODO 读取下/上 一页之前 先释放当前页的guard 因为可能上/下 一页阻塞在访问当前页的过程里
  ...
  std::lock_guard<std::mutex> __lock(tree_mutex_);
  Context ctx;
...
// 传入被插入页的guard 1.如果是叶子页 满了就建立新页然后进行split （1）此时如果无父页就进行建立内页 然后注册信息
// （2）如果有父页就进行pushup递归向上检测
FULL_INDEX_TEMPLATE_ARGUMENTS
  ...
  if (temp_write->IsLeafPage()) {
    ...
    if (page_write->GetSize() >= page_write->GetMaxSize()) {
      ...
      if (page_write->GetFatherPageId() == INVALID_PAGE_ID) {
        ...
        auto internal_page_write =
            internal_page_guard.AsMut<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
        ...
        // 设置internal页的键值对
        internal_page_write->FirstInsert(page_write_min_key, new_page_write->GetMinKey(), page_write_page_id,
        ...
        page_id_t parent_id = page_write->GetFatherPageId();

        write_guard.Drop();
        new_page_guard.Drop();

        // 分裂后将新页信息插入父页
        auto father_page_guard = bpm_->WritePage(parent_id);
        auto father_page_write =
            father_page_guard.template AsMut<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
    ...
    auto page_write = write_guard.AsMut<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
    if (GetInternalMaxSize() == page_write->GetSize()) {
      ...
      auto new_internal_write = new_internal_guard.AsMut<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
      ...
      if (parent_internal_page_id == INVALID_PAGE_ID) {
        ...
        auto parent_internal_write =
            parent_internal_guard.AsMut<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
        ...
        KeyType right_min_key{};
        {
          ...
          auto first_internal_write =
              first_internal_guard.template AsMut<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
        ...
        }
        {
          ...
          auto second_internal_write2 =
              second_internal_guard.AsMut<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
          second_internal_write2->SetFatherPageId(parent_internal_page_id);
          right_min_key = second_internal_write2->GetMinKey();
        }
        parent_internal_write->FirstInsert(left_min_key, right_min_key, first_internal_id, second_internal_id);
        auto head_guard = bpm_->WritePage(header_page_id_);
        auto head_write = head_guard.AsMut<BPlusTreeHeaderPage>();
...
FULL_INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::GetLeafMaxSize() -> int { return leaf_max_size_; }

FULL_INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::GetInternalMaxSize() -> int { return internal_max_size_; }
/*****************************************************************************
 * REMOVE
 *****************************************************************************/
...
FULL_INDEX_TEMPLATE_ARGUMENTS  // 可以加个需要分配多少的参数 然后加在判断条件上
    auto BPLUSTREE_TYPE::IsDistributeForInternal(
        BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator> *internal_write, int to_size) -> page_id_t {
  ...
  if (page_id == leaf_write->GetPrePageId()) {
    ...
    if (father_id != INVALID_PAGE_ID) {
      ...
      if (comparator_(new_first_key, old_first_key) != 0) {
        ...
        RecursiveUpdateKeyForRedistribute(old_first_key, std::make_pair(new_first_key, leaf_write->GetPageId()),
                                          father_write);
      }
    }
  } else {
    ...
    if (father_id != INVALID_PAGE_ID) {
      ...
      RecursiveUpdateKeyForRedistribute(old_right_first_key,
                                        std::make_pair(right_new_first_key, right_write->GetPageId()), father_write);
    }
    ...
    if (receiver_father_id != INVALID_PAGE_ID) {
      ...
      if (need_update) {
        ...
        RecursiveUpdateKeyForRedistribute(receiver_old_key, std::make_pair(new_first_key, leaf_write->GetPageId()),
...
FULL_INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREE_TYPE::RedistributeForInternal(page_id_t page_id,
  ...
  if (page_id == internal_write->GetPrePageId(father_write)) {
    ...
    RecursiveUpdateKeyForRedistribute(
        old_first_key, std::make_pair(internal_write->KeyAt(0), internal_write->GetPageId()), father_write);
  } else {
    // 右边处理逻辑
    auto right_guard = bpm_->WritePage(page_id);
    ...
    RecursiveUpdateKeyForRedistribute(old_right_first_key,
                                      std::make_pair(right_write->KeyAt(0), right_write->GetPageId()), father_write);
  }
}
...
void BPLUSTREE_TYPE::MergeForLeaf(WritePageGuard &leaf_guard) {
  ...
  if (left_id != INVALID_PAGE_ID) {
    ...
    if (left_write->GetFatherPageId() == leaf_write->GetFatherPageId()) {
      ...
      if (index == -1) {
        index = father_write->ValueIndexForPage_id_t(left_page_id_to_delete);
      }
      // 直接夺舍
      if (pre_size == 0) {
        if (index != -1) {
  ...
  if (right_id != INVALID_PAGE_ID) {
    ...
    if (leaf_write->GetFatherPageId() == right_write->GetFatherPageId()) {
      ...
      auto father_write = father_page.template AsMut<BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>>();
      if (pre_is_empty != 0) {
        auto index2 = father_write->MatchKey(begin_key, comparator_);
        if (index2 != -1) {
          father_write->DeletePair(index2);
        }
      } else {
        auto index2 = father_write->MatchKey(begin_key, comparator_);
        if (index2 != -1) {
          father_write->UpdateKey(index2, leaf_write->GetPageId());
        }
      }
...
FULL_INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREE_TYPE::DeepDeleteOrUpdate(const KeyType &key, std::optional<KeyType> update_key,
                                        BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator> *internal_write,
                                        bool is_update) {
  // 删除传入write的父页相关的信息
  auto father_id = internal_write->GetFatherPageId();
  ...
  if (father_index == -1) {
    father_index = father_write->ValueIndexForPage_id_t(internal_write->GetPageId());
  }
  if (father_index == -1) {
    return;
  }
...
void BPLUSTREE_TYPE::DeepUpdate(B_PLUS_TREE_LEAF_PAGE_TYPE *leaf_write, KeyType temp_key) {
  ...
  if (leaf_write->GetFatherPageId() != INVALID_PAGE_ID) {
    ...
    if (leaf_write->IsUpdate() && !leaf_write->IsEmpty()) {
      ...
      if (father_index == -1) {
        father_index = father_write->ValueIndexForPage_id_t(leaf_write->GetPageId());
      }
      if (father_index == -1) {
